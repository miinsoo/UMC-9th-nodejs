        ...
        try {
            // 가게가 존재하는지 확인
             if (!await this.storeRepository.getStoreById(reviewData.storeId)) {
                 throw new NotFoundError('가게를 찾을 수 없습니다.', reviewData);
            }
            ...
            if (imageUrl.length === reviewData.imgCount) {
                ...
                throw new InternalServerError('S3에서 충분한 이미지 url을 가져오지 못했습니다.', reviewData);
            }

            // 추가된 리뷰 정보 반환
            const review = await this.reviewRepository.getReviewById(newReview.id);
            if (!review) {
                throw new NotFoundError('리뷰를 찾을 수 없습니다.', reviewData);
            }
            ...
            console.error(error);
            throw new InternalServerError('리뷰 추가 중에 오류가 발생했습니다.', reviewData);
        }    
    }
}
