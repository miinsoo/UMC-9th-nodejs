...
class MissionService {
    ...
    async addMission(missionData) {
        try {
            ...
            if (!await this.storeRepository.getStoreById(missionData.storeId)) {
                throw new NotFoundError('가게를 찾을 수 없습니다.', missionData);
            }
            const newMission = await this.missionRepository.addMission(missionData);
            if (!newMission) {
                throw new NotFoundError('미션을 찾을 수 없습니다.', missionData);
            }
            ...
            console.error(error);
            throw new InternalServerError('미션 추가 중에 오류가 발생했습니다.', missionData);
        }
    }   
    async getMissionByStoreId(missionData) {
        try {
            ...
            console.error(error);
            throw new InternalServerError('미션 조회 중에 오류가 발생했습니다.', missionData);
        }
    }
}
